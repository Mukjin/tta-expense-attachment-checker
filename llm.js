...
import {
  SYSTEM_PROMPT, REVIEW_PROMPT, EXPENSE_PROMPT, DOC_OPEN, DOC_CLOSE, REVIEW_STATUSES, STATUS_UNKNOWN,
} from './prompts.js';

// ── 공통: 타임아웃·에러 매핑 ──
const IDLE_MS = 30000;   // 스트리밍 청크 간 최대 대기
const HARD_MS = 150000;  // 9B 로컬 모델 첫 응답 시간을 포함한 전체 상한
...
export async function inspectExpense(text, config, opts = {}) {
  return requestLlm(text, config, 'expense', opts);
}

async function requestLlm(text, config, mode, { onPartial, signal } = {}) {
  const sys = mode === 'review' ? REVIEW_PROMPT : mode === 'expense' ? EXPENSE_PROMPT : SYSTEM_PROMPT;
  ...
  try {
    raw = provider === 'gemini'
      ...
      : await callOpenAI(wrapped, config, sys, onAccum, signal, mode);
  } catch (e) {
    throw fetchError(e);
  }
  const parsed = mode === 'review' ? parseReview(raw) : mode === 'expense' ? parseExpense(raw) : parseResult(raw);
  if (mode === 'summary') {
    const hasContent = parsed.title || parsed.one_line || parsed.key_points?.length
      || parsed.actions?.length || parsed.cautions?.length;
    if (!hasContent) throw new Error('AI 요약 결과가 비어 있습니다. 문서 본문을 추출했는지 확인한 뒤 다시 시도하세요.');
  }
  if (mode === 'review') {
    const hasContent = parsed.summary || parsed.strengths?.length
      || parsed.improvements?.length || parsed.checks?.length;
    if (!hasContent) throw new Error('AI 검토 결과가 비어 있습니다. 문서 본문을 추출했는지 확인한 뒤 다시 시도하세요.');
  }
  return parsed;
}
...
async function callOpenAI(text, config, sys, onAccum, signal, mode = 'summary') {
  ...
  // 특정 모델을 저장하지 않은 첫 실행에서는 서버의 설치 모델을 자동 선택한다.
  // 존재하지 않을 수 있는 모델명을 하드코딩하면 Ollama가 정상이어도 404가 발생한다.
  if (!model) {
    const modelRes = await fetch(`${base}/v1/models`, {
      headers: auth,
      signal: withTimeout(signal),
    });
    if (!modelRes.ok) throw apiError(modelRes.status, await modelRes.text(), ctx);
    model = (((await modelRes.json()).data) || []).map((item) => item.id).find(Boolean);
    if (!model) throw new Error('서버에 설치된 모델이 없습니다. Ollama 모델을 먼저 설치하세요.');
  }
  const payload = {
    model,
    temperature: 0.2,
    max_tokens: mode === 'expense' ? 850 : 650, // 4K 컨텍스트·로컬 9B 속도에 맞춘 출력 상한
...
function parseExpense(raw) {
  ...
  try {
    const r = JSON.parse(stripped);
    const level = (value) => value === 'warn' ? 'warn' : 'info';
    const findings = Array.isArray(r.findings) ? r.findings.slice(0, 12).map((item) => ({
      level: level(item?.level),
      file: asText(item?.file).slice(0, 240),
      item: asText(item?.item).slice(0, 40),
      message: asText(item?.message).slice(0, 500),
      evidence: asText(item?.evidence).slice(0, 300),
    })).filter((item) => item.message) : [];
    const crossChecks = Array.isArray(r.cross_checks) ? r.cross_checks.slice(0, 8).map((item) => ({
      level: level(item?.level),
      item: asText(item?.item).slice(0, 80),
      message: asText(item?.message).slice(0, 500),
      files: asList(item?.files).slice(0, 8).map((file) => file.slice(0, 240)),
    })).filter((item) => item.message) : [];
    return {
      summary: asText(r.summary).slice(0, 800),
      findings,
      crossChecks,
      limitations: asList(r.limitations).slice(0, 8).map((item) => item.slice(0, 400)),
    };
  } catch {
    return {
      summary: 'AI 응답 형식을 확인하지 못해 의미 기반 검사를 반영하지 않음.',
      findings: [],
      crossChecks: [],
      limitations: ['AI가 유효한 JSON 형식으로 응답하지 않음.'],
    };
  }
}
