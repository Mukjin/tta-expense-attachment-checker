...
$('expenseBtn').addEventListener('click', () => {
  ...
  $('expenseBtn').textContent = '첨부검사 시작 중…';
  chrome.runtime.sendMessage({ type: 'edoc-expense-check-active', force: true }, (response) => {
    const error = chrome.runtime.lastError;
    if (error || !response?.ok) {
      $('expenseBtn').disabled = false;
      $('expenseBtn').textContent = `검사 시작 실패 — ${error?.message || response?.error || '알 수 없는 오류'}`;
      return;
    }
    window.close();
  });
