...
전자결재 화면에서 우클릭 **📎 지출결의 첨부 전체 검사**, 툴바 팝업의 첨부검사 버튼 또는
플로팅 버튼 옆 작은 **📎**를 누르면 다음 순서로 검사합니다.

1. 화면과 같은 출처의 모든 프레임에서 첨부파일명·다운로드 주소 수집
2. `expense-rules.json`에 등록된 승인 규칙으로 L1 필수첨부 존재 여부 판정
3. 첨부 원본을 메모리에서 로컬 파서로 전달해 L2 내용 추출
4. 금액·날짜·사업자번호·서명 표식을 규칙/정규식으로 추출
5. 로컬 LLM이 의미상 불일치 후보를 별도 경고로 제시

AI 결과는 차단 판정에 사용하지 않습니다. `통과 / 경고 / 차단`의 결정론적 판정과 AI 확인 후보를
패널에서 구분해 표시합니다. 파일 원문과 추출 전문은 디스크·브라우저 캐시에 저장하지 않습니다.

### 파일럿 자동 설치(권장)

`scripts\install-pilot.cmd`를 한 번 실행하면 다음 항목을 자동 구성합니다.

- `%LOCALAPPDATA%\TTAExpenseChecker\app` 고정 경로 설치
- 고정 확장 ID `lmkejmofkdcjnfcnmjgekbfippdklaco`
- Windows 로그인 시 문서 파서와 Ollama 무창 자동 기동
- `gw.tta.or.kr` 화면의 첨부검사 버튼 자동 표시

설치 중 자동으로 열린 `chrome://extensions`에서 개발자 모드를 켜고
`%LOCALAPPDATA%\TTAExpenseChecker\app` 폴더를 **압축해제된 확장 프로그램**으로 한 번만
로드합니다. 이후에는 확장 ID 입력이나 검은 창 실행이 필요 없습니다.

조직 배포용 ZIP·CRX 생성 방법과 파일럿 설치 상세는 [`distribution/README.md`](distribution/README.md)를 참고합니다.

### 개발자용 수동 실행

Node.js 20 이상에서 저장소 폴더를 열고 실행합니다.
...
파서는 `127.0.0.1:11435`에만 열리며 고정 확장 ID 하나의 요청만 허용합니다.

- 자동 텍스트 추출: HWP 3.x/5.x, HWPX, HWPML, PDF, XLS, XLSX, DOCX
- 이미지·스캔 PDF: 현재 PoC는 OCR 선택 모듈을 설치하지 않아 `OCR 필요`로 차단
- 암호화·DRM 문서: 자동 해제하지 않고 검사불가로 차단
- 첨부 다운로드 주소가 화면 DOM에 없을 때: 패널의 **검사불가 파일 직접 선택**으로 같은 창에서 보완
- 파일 크기: 파일당 32MB, 한 번에 최대 30개

`expense-rules.json`은 `expense-rules.schema.json` 형식에 따르며 실제 승인된 지출유형별 규칙을
받기 전까지 빈 규칙으로 유지됩니다. 확장 설정 화면의 **지출유형별 첨부 규칙**에서 승인된 JSON을
붙여넣으면 조건·정규식·대체 증빙 참조를 검증한 뒤 이 기기의 `chrome.storage.local`에 저장합니다.
규칙이
비어 있으면 파일 내용은 읽되 필수증빙 충족을 확정하지 않고 `적용 규칙 미설정` 경고를 표시합니다.
