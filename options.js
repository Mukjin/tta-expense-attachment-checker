...
import { validateExpenseRules } from './expense-checker.js';
...
document.addEventListener('DOMContentLoaded', async () => {
  const [c, storedRules, bundledRules] = await Promise.all([
    loadConfig(),
    chrome.storage.local.get('expenseRules'),
    fetch(chrome.runtime.getURL('expense-rules.json'), { cache: 'no-store' }).then((response) => response.json()),
  ]);
  ...
  $('expenseRules').value = JSON.stringify(storedRules.expenseRules || bundledRules, null, 2);
});
...
$('ruleSaveBtn').addEventListener('click', async () => {
  ...
  let rules;
  try {
    rules = JSON.parse($('expenseRules').value);
  } catch (error) {
    out.textContent = `✗ JSON 형식 오류: ${error.message}`;
    return;
  }
  const validation = validateExpenseRules(rules);
  if (!validation.valid) {
    out.textContent = `✗ ${validation.errors.slice(0, 8).join(' · ')}`;
    return;
  }
  await chrome.storage.local.set({ expenseRules: rules });
  $('expenseRules').value = JSON.stringify(rules, null, 2);
  out.textContent = validation.warnings.length
    ? `저장됨 ✓ · 주의: ${validation.warnings.join(' · ')}`
    : '검증 및 저장 완료 ✓';
});

$('ruleResetBtn').addEventListener('click', async () => {
  await chrome.storage.local.remove('expenseRules');
  const bundled = await fetch(chrome.runtime.getURL('expense-rules.json'), { cache: 'no-store' }).then((response) => response.json());
  $('expenseRules').value = JSON.stringify(bundled, null, 2);
  $('ruleResult').textContent = '번들 규칙으로 초기화됨 ✓';
});
