...
import { summarize, review, inspectExpense } from './llm.js';
import { pickSource, sameOriginFrames, clampText, MAX_CHARS } from './pick.js';
import { loadConfig, hostPatterns } from './config.js';
import { buildExpenseMarkdown, buildMarkdown, buildReviewMarkdown, resultFileName } from './md.js';
import { renderOverlay } from './overlay.js';
import { checkExpenseAttachments, loadExpenseRules, normalizeAttachments } from './expense-checker.js';
import { parseAllAttachments, parseUploadedAttachment } from './attachment-reader.js';
import { applyContentResults, applyExpenseAiReview, buildExpenseAiInput } from './expense-content-checker.js';
import { inferExpenseContext } from './expense-context.js';

const MENU_SUM = 'edoc-summarize';
const MENU_REV = 'edoc-review';
const MENU_EXPENSE = 'edoc-expense-check';
...
chrome.contextMenus.onClicked.addListener((info, tab) => {
  ...
  else if (info.menuItemId === MENU_EXPENSE) runExpenseCheck(tab.id);
});
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg?.type === 'edoc-expense-check-active') {
    chrome.tabs.query({ active: true, currentWindow: true }).then(([tab]) => {
      if (!tab?.id) return sendResponse({ ok: false, error: '현재 탭을 찾지 못했습니다.' });
      runExpenseCheck(tab.id, { force: msg?.force === true });
  ...
  if (msg?.type === 'edoc-expense-upload-start') {
    sendResponse(startExpenseUpload(tabId, msg));
    return;
  }
  if (msg?.type === 'edoc-expense-upload-chunk') {
    sendResponse(appendExpenseUpload(tabId, msg));
    return;
  }
  if (msg?.type === 'edoc-expense-upload-end') {
    finishExpenseUpload(tabId, msg).then(sendResponse, (error) => sendResponse({ ok: false, error: error.message }));
    return true;
  }
  if (msg?.type === 'edoc-expense-upload-commit') {
    commitExpenseUpload(tabId, msg).then(sendResponse, (error) => sendResponse({ ok: false, error: error.message }));
    return true;
  }
  if (msg?.type === 'edoc-expense-upload-cancel') {
    uploadBatches.delete(`${tabId}:${String(msg.batchId || '')}`);
    sendResponse({ ok: true });
    return;
  }
  const force = msg?.force === true;
  if (msg?.type === 'edoc-summarize') runSummarize(tabId, 'summary', { force });
  else if (msg?.type === 'edoc-review') runSummarize(tabId, 'review', { force });
  else if (msg?.type === 'edoc-expense-check') runExpenseCheck(tabId, { force });
});

// 탭이 닫히면 해당 탭의 결과 캐시 정리 + 진행 중 요청 중단
chrome.tabs.onRemoved.addListener((tabId) => {
  chrome.storage.session.remove([`cache_${tabId}_summary`, `cache_${tabId}_review`, `cache_${tabId}_expense`]);
  inflight.get(tabId)?.controller.abort();
  inflight.delete(tabId);
  for (const [key, batch] of uploadBatches) {
    if (batch.tabId === tabId) uploadBatches.delete(key);
  }
});
...
const MAX_UPLOAD_CHUNK_CHARS = 400000;

function uploadKey(tabId, batchId) {
  return `${tabId}:${String(batchId || '').slice(0, 100)}`;
}

function startExpenseUpload(tabId, msg) {
  const expiry = Date.now() - 10 * 60 * 1000;
  for (const [key, batch] of uploadBatches) {
    if (batch.createdAt < expiry) uploadBatches.delete(key);
  }
  const batchId = String(msg.batchId || '').slice(0, 100);
  const uploadId = String(msg.uploadId || '').slice(0, 100);
  const name = String(msg.name || '').replace(/[\r\n\0]/g, '').slice(0, 240);
  const size = Number(msg.size || 0);
  if (!batchId || !uploadId || !name) return { ok: false, error: '업로드 식별값이 없습니다.' };
  if (!Number.isInteger(size) || size <= 0 || size > MAX_UPLOAD_BYTES) {
    return { ok: false, error: '파일 크기가 비어 있거나 32MB 제한을 초과했습니다.' };
  }
  const key = uploadKey(tabId, batchId);
  let batch = uploadBatches.get(key);
  if (!batch) {
    batch = { tabId, createdAt: Date.now(), files: new Map(), results: [] };
    uploadBatches.set(key, batch);
  }
  if (batch.files.size + batch.results.length >= 30) return { ok: false, error: '한 번에 최대 30개 파일을 검사할 수 있습니다.' };
  batch.files.set(uploadId, {
    id: String(msg.attachmentId || '').slice(0, 240), name, size, chunks: [], chars: 0, nextSequence: 0,
  });
  return { ok: true };
}

function appendExpenseUpload(tabId, msg) {
  const batch = uploadBatches.get(uploadKey(tabId, msg.batchId));
  const uploadId = String(msg.uploadId || '').slice(0, 100);
  const file = batch?.files.get(uploadId);
  const chunk = String(msg.chunk || '');
  const sequence = Number(msg.sequence);
  if (!file) return { ok: false, error: '업로드 세션이 만료되었습니다.' };
  if (sequence !== file.nextSequence) return { ok: false, error: '파일 조각 순서가 올바르지 않습니다.' };
  if (!chunk || chunk.length > MAX_UPLOAD_CHUNK_CHARS || !/^[A-Za-z0-9+/]*={0,2}$/.test(chunk)) {
    return { ok: false, error: '파일 조각 형식이 올바르지 않습니다.' };
  }
  file.chunks.push(chunk);
  file.chars += chunk.length;
  file.nextSequence += 1;
  if (file.chars > Math.ceil(MAX_UPLOAD_BYTES * 4 / 3) + 16) {
    batch.files.delete(uploadId);
    return { ok: false, error: '파일 조각이 32MB 제한을 초과했습니다.' };
  }
  return { ok: true };
}

async function finishExpenseUpload(tabId, msg) {
  const key = uploadKey(tabId, msg.batchId);
  const batch = uploadBatches.get(key);
  const uploadId = String(msg.uploadId || '').slice(0, 100);
  const file = batch?.files.get(uploadId);
  if (!file) return { ok: false, error: '업로드 세션이 만료되었습니다.' };
  batch.files.delete(uploadId);
  const expectedChars = Math.ceil(file.size / 3) * 4;
  if (file.chars !== expectedChars) return { ok: false, error: '파일 조각이 누락되었거나 크기가 일치하지 않습니다.' };
  const result = await parseUploadedAttachment({
    id: file.id,
    name: file.name,
    contentBase64: file.chunks.join(''),
  });
  batch.results.push(result);
  return { ok: true, success: result.success === true, name: file.name, error: result.error || '' };
}

async function commitExpenseUpload(tabId, msg) {
  const key = uploadKey(tabId, msg.batchId);
  const batch = uploadBatches.get(key);
  if (!batch || batch.files.size) return { ok: false, error: '아직 처리 중인 파일이 있거나 업로드 세션이 만료되었습니다.' };
  uploadBatches.delete(key);
  await mergeUploadedExpenseResults(tabId, batch.results);
  return { ok: true, count: batch.results.length };
}
...
function isTtaApprovalPage(url) {
  try {
    const parsed = new URL(url);
    return parsed.hostname.toLowerCase() === 'gw.tta.or.kr'
      && /\/gwweb\/appr\//i.test(parsed.pathname);
  } catch {
    return false;
  }
}

// 지출결의 첨부 L1 검사: 현재 미리보기 파일이 아니라 같은 출처 프레임의 첨부 목록 전체를 합친다.
async function runExpenseCheck(tabId, { force = false } = {}) {
  inflight.get(tabId)?.controller.abort();
  const controller = new AbortController();
  inflight.set(tabId, { mode: 'expense', controller });
  const aborted = () => controller.signal.aborted;
  const overlay = (state) => chrome.scripting.executeScript({
    target: { tabId },
    func: renderOverlay,
    args: [{ mode: 'expense', gen: nextGen(), ...state }],
  }).catch((e) => { console.warn('[edoc] 첨부검사 오버레이 주입 실패:', e.message); });

  try {
    await overlay({ status: '첨부 목록 전체 확인 중…' });
    const tab = await chrome.tabs.get(tabId).catch(() => null);
    const tabUrl = tab?.url || '';
    let topOrigin = '';
    try { topOrigin = new URL(tabUrl).origin; } catch { /* edge:// 등 */ }

    const [results, contextResults] = await Promise.all([
      chrome.scripting.executeScript({
        target: { tabId, allFrames: true },
        files: ['attachment-extractor.js'],
      }),
      chrome.scripting.executeScript({
        target: { tabId, allFrames: true },
        files: ['expense-context-extractor.js'],
      }),
    ]);
    if (aborted()) return;

    const frames = results.map((item) => item.result).filter(Boolean);
    const allowed = frames.filter((frame) => !topOrigin || frame.origin === topOrigin);
    const dropped = frames.filter((frame) => topOrigin && frame.origin !== topOrigin && frame.attachments?.length);
    const attachments = normalizeAttachments(allowed.flatMap((frame) => frame.attachments || []));
    const contextFrames = contextResults.map((item) => item.result).filter((frame) =>
      frame && (!topOrigin || frame.origin === topOrigin));
    const expenseContext = inferExpenseContext(contextFrames, { url: tabUrl, title: tab?.title || '' });
    const hash = fingerprint(JSON.stringify(attachments.map((item) => ({
      id: item.id,
      name: item.name,
      href: item.href,
      sizeText: item.sizeText,
    }))));
    const cacheKey = `cache_${tabId}_expense`;
    if (!force) {
    ...
    await overlay({ status: `첨부 ${attachments.length.toLocaleString()}건 규칙 검사 중…` });
    const ruleset = await loadExpenseRules();
    if (aborted()) return;
    // 지출유형 등 업무 필드는 실제 작성 화면 구조 확인 후 context에 추가한다.
    let check = checkExpenseAttachments(expenseContext, attachments, ruleset);
    const warnings = [];
    if (attachments.length) {
      const parsed = await parseAllAttachments(attachments, {
        signal: controller.signal,
        onProgress: (current, total, attachment) => overlay({
          status: `첨부 내부 읽는 중… (${current}/${total}) ${attachment.name}`,
        }),
      });
      if (aborted()) return;
      check = applyContentResults(check, parsed);

      // 첨부 원문은 로컬 LLM에만 전달한다. 설정이 Gemini여도 지출 증빙을 외부로 보내지 않는다.
      const aiInput = buildExpenseAiInput(parsed);
      if (aiInput) {
        const localConfig = await loadConfig();
        if (localConfig.provider === 'gemini') {
          warnings.push('지출 증빙 AI 검사는 외부 전송을 막기 위해 건너뛰었습니다. 설정에서 로컬 LLM을 선택하면 AI 의미 검사를 추가합니다.');
        } else {
          await overlay({ status: '로컬 AI가 첨부 간 금액·날짜·서명 후보를 확인 중…' });
          try {
            check = applyExpenseAiReview(check, await inspectExpense(aiInput, localConfig, {
              signal: controller.signal,
            }));
          } catch (error) {
            if (error?.cancelled || aborted()) return;
            warnings.push(`로컬 AI 의미 검사를 완료하지 못했습니다: ${error.message}`);
          }
        }
      }
    }
    if (dropped.length) warnings.push(`다른 출처 프레임의 첨부 후보 ${dropped.length}개는 보안상 제외했습니다.`);
    if (!check.rulesetId) warnings.push('승인된 지출유형별 규칙 파일이 아직 없어 목록 존재 여부만 확인했습니다.');
    const finalState = {
      expenseCheck: check,
      md: buildExpenseMarkdown(check),
      fileName: resultFileName('첨부검사', tab?.title || '지출결의'),
      warn: warnings.join(' '),
    };
    if (aborted()) return;
    ...
    if (aborted()) return;
    await overlay({ error: '첨부검사 실패: ' + e.message });
  } finally {
    if (inflight.get(tabId)?.controller === controller) inflight.delete(tabId);
  }
}

// 레거시 화면에서 다운로드 URL을 찾지 못했을 때, 사용자가 같은 오버레이에서 직접 선택한
// 파일의 로컬 파싱 결과만 합친다. 원문 markdown은 이 함수가 끝난 뒤 캐시하지 않는다.
async function mergeUploadedExpenseResults(tabId, incoming) {
  const results = Array.isArray(incoming) ? incoming.slice(0, 30).map((item) => ({
    id: String(item?.id || '').slice(0, 240),
    name: String(item?.name || '').replace(/[\r\n\0]/g, '').slice(0, 240),
    success: item?.success === true,
    fileType: String(item?.fileType || '').slice(0, 30),
    pageCount: Number(item?.pageCount || 0),
    isImageBased: item?.isImageBased === true,
    markdown: String(item?.markdown || '').slice(0, 500000),
    textChars: Number(item?.textChars || 0),
    truncated: item?.truncated === true,
    warnings: Array.isArray(item?.warnings) ? item.warnings.slice(0, 100) : [],
    qualitySummary: item?.qualitySummary || null,
  ...
  const cacheKey = `cache_${tabId}_expense`;
  const { [cacheKey]: hit } = await chrome.storage.session.get(cacheKey);
  if (!hit?.state?.expenseCheck) return;
  const overlay = (state) => chrome.scripting.executeScript({
    target: { tabId },
    func: renderOverlay,
    args: [{ mode: 'expense', gen: nextGen(), ...state }],
  }).catch((error) => console.warn('[edoc] 직접선택 검사 표시 실패:', error.message));

  await overlay({ status: `직접 선택한 첨부 ${results.length}건 결과 반영 중…` });
  let check = applyContentResults(hit.state.expenseCheck, results, { supplemental: true });
  let extraWarning = '다운로드 주소를 찾지 못한 파일은 사용자가 직접 선택한 원본으로 검사했습니다.';
  const aiInput = buildExpenseAiInput(results);
  if (aiInput) {
    const localConfig = await loadConfig();
    if (localConfig.provider !== 'gemini') {
      try {
        ...
        extraWarning += ` 로컬 AI 의미 검사는 완료하지 못했습니다: ${error.message}`;
      }
    }
  }
  const state = {
    ...
    expenseCheck: check,
    md: buildExpenseMarkdown(check),
    warn: [hit.state.warn, extraWarning].filter(Boolean).join(' '),
  };
  await chrome.storage.session.set({ [cacheKey]: { ...hit, state } });
  await overlay(state);
}
...
async function runSummarize(tabId, mode = 'summary', { force = false } = {}) {
  ...
  try {
    ...
    const ttaApprovalPage = isTtaApprovalPage(tabUrl);

    // DOM에서 공문 본문을 못 얻은 경우(캔버스 렌더링 웹한글): MAIN 월드에서 뷰어 JS API 프로브
    let probeDebug = '';
    if (!source || source.how === '문서카드' || (ttaApprovalPage && source.how === '페이지 전체')) {
    ...
    // TTA 결재 화면의 최상위 bodyText는 툴바·결재선·첨부파일명으로 대부분 구성됨.
    // 웹한글 캔버스 본문 API도 얻지 못했다면 이를 문서 원문으로 AI에 보내지 않는다.
    if (ttaApprovalPage && source?.how === '페이지 전체') {
      const debug = probeDebug ? ` [API 후보: ${probeDebug}]` : '';
      await overlay({
        error: '본문 추출 실패 — TTA 웹한글 본문을 읽지 못했습니다. 원기안문 또는 PDF 보기에서 본문 텍스트를 드래그 선택한 뒤 다시 실행하세요.' + debug,
      });
      return;
    }
    if (!source) {
