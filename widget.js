// 전자문서 화면에 플로팅 지출 첨부검사 버튼을 띄우는 콘텐츠 스크립트.
// 큰 버튼은 첨부검사, 작은 보조 버튼은 AI 요약. 드래그로 위치 이동 가능(위치는 저장되어 유지).
//
...
(async () => {
  ...
  const DEFAULT_HOSTS = ['gw.tta.or.kr', 'cseoul.go.kr'];
  ...
  const tryMount = async () => {
    ...
    root.innerHTML = `
      <style>
        ...
        .core { color:#fff; font: 24px/1 "Segoe UI Emoji", "Apple Color Emoji", sans-serif; }
        .orb:hover::before { opacity: .75; }
        .orb:hover .core { transform: scale(1.06); }
        .orb:active .core { transform: scale(.95); }
        .secondary {
          all: initial; position: absolute; right: -7px; bottom: -10px; z-index: 2;
          width: 29px; height: 29px; border-radius: 10px; cursor: pointer;
          display: flex; align-items: center; justify-content: center;
          background: #fff; color: #0A4DA2; border: 1px solid rgba(10, 77, 162, .2);
          box-shadow: 0 4px 12px rgba(11, 27, 51, .2); font-size: 15px;
          transition: transform .15s ease;
        }
        .secondary:hover { transform: scale(1.08); }
        @keyframes spin { to { transform: rotate(360deg); } }
        @media (prefers-reduced-motion: reduce) {
          .orb::before { animation: none; }
        }
      </style>
      <button class="orb" title="지출결의 첨부 전체 검사 (드래그로 이동)">
        <span class="core" aria-hidden="true">📎</span>
      </button>
      <button class="secondary" title="AI 문서 요약">✨</button>`;

    // 큰 버튼 클릭 = 첨부검사, 드래그(5px 이상 이동) = 위치 이동
    const btn = root.querySelector('.orb');
    const summaryBtn = root.querySelector('.secondary');
    summaryBtn.addEventListener('click', (event) => {
      if (!event.isTrusted) return;
      try {
        chrome.runtime.sendMessage({ type: 'edoc-summarize' }, () => void chrome.runtime.lastError);
      } catch {
        summaryBtn.title = '확장이 업데이트되었습니다 — 페이지를 새로고침하세요';
      }
    });
    btn.addEventListener('pointerdown', (e) => {
      ...
      const up = (ev) => {
        ...
        if (dragged) {
          ...
          try {
            chrome.runtime.sendMessage({ type: 'edoc-expense-check' }, () => void chrome.runtime.lastError);
          } catch {
